...
namespace KINEMATION.ProceduralRecoilAnimationSystem.Runtime
{
    ...
    [AddComponentMenu(RecoilMenuPath)]
    public class RecoilAnimation : MonoBehaviour
    {
        public const string RecoilMenuPath = "KINEMATION/Recoil Animation";
        
        public KTransform RecoilTransform => new KTransform(OutLoc, OutRot, Vector3.one);
        ...
        private void SetupStateMachine()
        {
            ...
            autoState.onPlay = () =>
            {
                ...
                bool hasValidAutoCurves = curves.autoRotCurve.IsValid() && curves.autoLocCurve.IsValid();
                bool hasValidSemiCurves = curves.semiRotCurve.IsValid() && curves.semiLocCurve.IsValid();

                _enableSmoothing = hasValidAutoCurves || hasValidSemiCurves;
                float correction = 60f / _fireRate;

                if (hasValidAutoCurves)
                {
