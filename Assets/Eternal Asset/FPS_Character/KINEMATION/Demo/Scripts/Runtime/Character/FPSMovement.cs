...
namespace Demo.Scripts.Runtime.Character
{
    ...
    public class FPSMovement : MonoBehaviour
    {
        ...
        private void Prone()
        {
            float proneHeight = _originalHeight * movementSettings.proneRatio;
            float heightDifference = _originalHeight - proneHeight;

            _controller.height = proneHeight;

            // Adjust the center position so the bottom of the capsule remains at the same position
            Vector3 proneCenter = _originalCenter;
            proneCenter.y -= heightDifference / 2;
            _controller.center = proneCenter;
        }
        ...
        private void UpdateMovementState()
        {
            ...
            if (MovementState == FPSMovementState.Sprinting 
                && _inputDirection.y > 0f && Mathf.Approximately(_inputDirection.x, 0f) /*&& !GetComponent<SurvivalSystem>().IsBusy*/)
            {
                //GetComponent<SurvivalSystem>().OnSprintConsume(Time.deltaTime);
                return;
            }
            ...
            //GetComponent<SurvivalSystem>().OnWalkConsume(Time.deltaTime);
        }
        ...
        private void Start()
        {
            movementSettings.walking.velocity = 3f;
            movementSettings.sprinting.velocity = 6.5f;
        ...
        {
            var _playerInputSystem = GetComponent<PlayerInput>();

            _playerInputSystem.actions["Move"].performed += OnMove;
            _playerInputSystem.actions["Crouch"].performed += OnCrouch;
            _playerInputSystem.actions["Prone"].performed += OnProne;
            _playerInputSystem.actions["Sprint"].performed += OnSprint;
            _playerInputSystem.actions["Slide"].performed += OnSlide;
            _playerInputSystem.actions["Jump"].performed += OnJump;
            _playerInputSystem.actions["Move"].canceled += OnMove;
            _playerInputSystem.actions["Sprint"].canceled += OnSprint;
        }
        ...
        public void Encombrant(bool v)
        {
            if (v)
            {
                movementSettings.walking.velocity -= 1.5f;
                movementSettings.sprinting.velocity -= 3f;
        ...
        public void OnProne(InputAction.CallbackContext ctx)
        {
            if (!_consumeMoveInput) return;
        ...
        {
            /*Debug.Log(GetComponent<SurvivalSystem>().IsBusy);
            if (!_consumeMoveInput || MovementState == FPSMovementState.InAir || PoseState == FPSPoseState.Crouching || GetComponent<SurvivalSystem>().IsBusy)
            {
                return;
            }
            ...
            GetComponent<SurvivalSystem>().OnSprintConsume(1);
        ...
        public void OnSprint(InputAction.CallbackContext ctx)
        {
            ...
            bool enableSprint = ctx.performed && CanSprint();

            /*if (enableSprint && !GetComponent<SurvivalSystem>().IsBusy)
            {
        ...
        {
            /*if (!_consumeMoveInput || !CanSlide() || GetComponent<SurvivalSystem>().IsBusy)
            {
                return;
            }

            GetComponent<SurvivalSystem>().OnSprintConsume(.25f);*/
