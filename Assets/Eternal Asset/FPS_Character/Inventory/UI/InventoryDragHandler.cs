...
/// <summary>
/// Gère le drag & drop des slots d'inventaire.
/// À attacher sur le même GameObject que InventorySlotUI, ou sur un enfant.
/// </summary>
...
public class InventoryDragHandler : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private void Awake()
    {
        ...
        if (_slotUI == null) _slotUI = GetComponentInParent<InventorySlotUI>(); // default get parent
    ...
    {
        if (_slotUI == null || _slotUI.Inventory == null) return;

        var stack = _slotUI.Inventory.GetStack(_slotUI.SlotIndex);
        if (stack.IsEmpty)
        {
